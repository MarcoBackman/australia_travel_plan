(() => {
 ...
 const booked=inputs.find(x=>x.dataset.check==='domestic');if(booked){booked.disabled=true;booked.closest('label').querySelector('b').textContent='02. 국내선 JQ959 예약 완료';booked.closest('label').querySelector('small').textContent='10/19 Jetstar Airways · CNS 13:30 → SYD 17:25 (각 도시 현지 시각). 1인 위탁 30kg·기내 12kg 구매 확정.';}
 const hotels=inputs.find(x=>x.dataset.check==='hotels');if(hotels)hotels.closest('label').querySelector('small').textContent='전체 숙소 300만원 이내. 포트더글라스 5박·시드니 4박과 도착일 오전 객실 확보를 포함해 최종 청구액·입실 조건 확인.';
