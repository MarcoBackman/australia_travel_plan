(() => {
  ...
  function setupContent(){
    ...
    const bag=body.querySelector('#bag-result');if(bag&&!initialized.has(bag))loadScript('assets/baggage.js').then(()=>{if(body.contains(bag)&&!initialized.has(bag)){window.initBaggageGuide();initialized.add(bag);}}).catch(()=>{bag.textContent='계산기를 불러오지 못했습니다. 모달을 다시 열어주세요.';});
    const entry=body.querySelector('#entry-copy-inquiry');if(entry&&!initialized.has(entry))loadScript('assets/entry.js').then(()=>{if(body.contains(entry)&&!initialized.has(entry)){window.initEntryPreparation();initialized.add(entry);}}).catch(()=>{});
  }
  ...
  window.openTripInfo=(heading,content,trigger)=>{if(dialog.open)dialog.close();restore();returnFocus=trigger||document.activeElement;title.textContent=heading;if(typeof content==='string')body.innerHTML=content;else body.append(content);dialog.showModal();dialog.scrollTop=0;setupContent();};
  function openTemplate(id,heading,trigger){const template=document.getElementById(id);if(!template)return;if(activeId===id&&dialog.open)return;if(dialog.open)dialog.close();restore();returnFocus=trigger||document.activeElement;title.textContent=heading;held={template};activeId=id;body.append(template.content);dialog.showModal();dialog.scrollTop=0;setupContent();}
